...
export const UserService = new class UserServiceClass {
    ...
    public LoadUser = (): Observable<UserContracts.User> => {
        ...
        const token: string = `Bearer ${this.store.getValue().accessToken}`;
        const config: AxiosRequestConfig = {
            headers: {
                "Content-Type": "application/json",
                Authorization: token
    ...
    public LoginUser = (data: UserContracts.NewUser): Observable<UserContracts.User> => {
        this.store.setLoading(true);
        return from(axios.post<UserContracts.User>("http://localhost:3000/login", data))
            .pipe(
                tap(response => {
                    console.log(response);
                    // this.RefreshTokenTimer.SetTimeout();
    ...
    public RegisterUser = (data: UserContracts.NewUser): Observable<UserContracts.User> => {
        this.store.setLoading(true);
        return from(axios.post<UserContracts.User>("http://localhost:3000/register", data))
            .pipe(
                tap(response => {
                    console.log(response);
    ...
    //     this.store.setLoading(true);
    //     try {
    //         const accessToken: string = `Bearer ${this.store.getValue().accessToken}`;
    //         const config: AxiosRequestConfig = {
    //             headers: {
    //                 "Content-Type": "application/json",
